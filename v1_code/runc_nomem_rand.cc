...
// Nov 2020 Version that randomly presents stimuli (i.e. not permuting)
// More like original BCM



DEFUN_DLD(runc_nomem_rand,args,nargout,"help? no way"){
    ...
    const bool blockupdateQ= (tau_th<0 ? 1: 0); // negative tau_th => use blocks
    ...
    for (long long int idt=1; idt < ndt ; idt ++){
        ...
        if (blockupdateQ==1){
            // theta update, fixed length blocks
            ...
            //double thlast = (idt==1 ? th0 : th_ar(idt-1));
           th_running = th_previous+ 1./tau_th*( -th_previous +  y_current*y_current);
           th_previous= th_running;
           
           if (idt%10000==0)
          printf("%g %g %g \n", y_current, th_previous, th_running);
          // if (idt > 20)
          //exit(1);
        ...
        for (int isnap=0; isnap < nsnaps; isnap++){
            if (idt==(long long int)snaptimes(isnap)){
                printf("Runc_nomem_rand.cc: snapshot at time %lli (%lli) = %g th= %g \n",idt,ndt,idt/(float)ndt,th_running);
                for (int i=0; i< N; i++)
                    w_snaps(i,isnap)= w(i);
